...
impl SignalHandler for JSSignalHandler {
    fn handle(&self, vm: &mut VM, arg: u16) -> Result<(), String> {
        ...
        let res = self.handler
            ...
            .map_err(|_| format!("uh oh"))?;
        if res.is_null() {
            Ok(())
        } else {
            let actions: VMActionSet = serde_wasm_bindgen::from_value(res).map_err(|x| format!("{x}"))?;
            ...
            let js_vec = serde_wasm_bindgen::to_value(&out).map_err(|x| format!("{x}"))?;
...
impl VMActionSet {
    ...
    pub fn write_memory(&mut self, addr: u32, val: u8) {
        self.0.push(VMAction::MemoryWrite(addr, val));
    }
}
