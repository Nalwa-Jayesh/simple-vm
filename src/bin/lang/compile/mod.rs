use crate::ast;
use simplevm::{
    Instruction, Literal10Bit, Literal12Bit, Literal7Bit, Nibble, Register, StackOp, TestOp,
};
    ...
    for s in statements {
        match s {
            ...
            ast::Statement::Declare(id, _t, Some(expr)) => {
                ...
                out.push(UnresolvedInstruction::Instruction(Instruction::Store(
                    Register::B,
                    Register::Zero,
                    Register::C,
                )));
            ...
            if let Some(v) = block.scope.get(&s) {
                match v {
                    BlockVariable::Local(i) => Ok(vec![
                        ...
                        UnresolvedInstruction::Instruction(Instruction::Load(
                            Register::C,
                            Register::C,
                            Register::Zero,
                        )),
    ...
    for p in program {
        match p {
            ast::TopLevel::FunctionDefinition { name, body, .. } => {
                ...
                ctx.define(
                    &Symbol::new(&local_count_sym),
                    block.scope.locals.len() as u32 * 2,
                );
