...
fn parse_type(input: &str) -> Result<(&str, ast::Type), ParseError> {
    ...
    let tt = ast::Type::from_str(&res)
        .map_err(|_s| ParseError::new(input, ParseErrorKind::ExpectedType))?;
    Ok((s, tt))
}
...
fn expression_lhs(input: &str) -> Result<(&str, ast::Expression), ParseError> {
    require(
        ...
        ParseError::new(input, ParseErrorKind::ExpectedExpressionLHS),
    )
    .run(input)
}
...
fn expression(input: &str) -> Result<(&str, ast::Expression), ParseError> {
    require(
        Any::new(vec![expression_binop, expression_lhs]),
        ParseError::new(input, ParseErrorKind::ExpectedExpression).tag("expression"),
    )
    .run(input)
}
...
pub fn parse_ast(input: &str) -> Result<(&str, Vec<ast::TopLevel>), ParseError> {
    ...
    loop {
        ...
        if state_next.is_empty() {
            ...
            let (snn, res) = AnyCollectErr::new(vec![function_definition])
                .run(state_next)
